...
import { addMetadataToFrontmatter, splitFrontMatter } from '../../frontmatter';
...
export class DiagramRegenerator {
  ...
  private static async processReferenceFiles(
    ...
    for (const reference of allReferences) {
      ...
      if (relativeFilePath.startsWith('/')) {
        relativeFilePath = relativeFilePath.slice(1); // remove leading slash
      }
  
      const absolutePath = workspacePath
        ? path.join(workspacePath, relativeFilePath)
        : relativeFilePath;
  
      const isChanged = changedFiles.some(
        cf => cf === fileName || cf === `${fileName} (deleted)`
      );
  ...
  private static async getUpdatedDiagramFromAI(prompt: string): Promise<any> {
    try {
      ...
      const configuredFamily = config.get<string>('mermaidChart.aiModelFamily') || config.get<string>('preview.mermaidChart.aiModelFamily') || 'gpt-4o';
  ...
  private static async updateDocument(
    ...
    try {
      ...
      if (uri.scheme === 'untitled') {
        ...
        tempDir = vscode.Uri.file(path.join(workspaceFolders[0].uri.fsPath, '.temp'));
        tempUri = vscode.Uri.file(path.join(tempDir.fsPath, `untitled-diagram-${Date.now()}.new`));
      } else {
        // For regular files, use the containing directory
        tempDir = vscode.Uri.file(path.join(uri.with({ path: uri.path.split('/').slice(0, -1).join('/') }).fsPath, '.temp'));

        tempUri = vscode.Uri.file(path.join(tempDir.fsPath, `${path.basename(uri.fsPath)}.new`));
      }
      ...
      if (choice === applyButton) {
        ...
        if (uri.scheme === 'untitled') {
          originalTempUri = vscode.Uri.file(path.join(tempDir.fsPath, `untitled-diagram-${Date.now()}.original`));
        } else {
          originalTempUri = vscode.Uri.file(path.join(tempDir.fsPath, `${path.basename(uri.fsPath)}.original`));
        }
